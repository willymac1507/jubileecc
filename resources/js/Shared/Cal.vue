...
let selectedDate = ref(
    props.selectDate ? new Date(props.selectDate) : new Date()
);
...
let firstMonday = isMonday(startOfMonth(today))
    ? startOfMonth(today)
    : previousMonday(startOfMonth(today));

let lastSunday = isSunday(lastDayOfMonth(today))
    ? lastDayOfMonth(today)
    : nextSunday(lastDayOfMonth(today));
...
<template>
    <div class="border rounded-md basis-9/12 flex max-h-[32rem] flex-col">
            ...
            <div>
                <h1 class="text-lg font-semibold leading-6 text-gray-900">
                    <time
                        ...
                        >{{ displayDate }}</time
                    >
                    <time :datetime="displayDate" class="hidden sm:inline">{{
                        displayDate
                    }}</time>
                </h1>
        ...
        <div class="isolate flex flex-auto overflow-hidden bg-white">
            <div
                ref="container"
                class="flex flex-auto flex-col max-h-full overflow-auto"
            >
                <div
                    ref="containerNav"
                ...
                <div class="flex w-full flex-auto">
                    ...
                    <div class="grid flex-auto grid-cols-1 grid-rows-1">
                        ...
                        <div
                            ...
                            style="grid-template-rows: repeat(32,minmax(3.5rem, 1fr));"
                        >
                            <div
                                ref="containerOffset"
                                class="row-end-1 h-7"
                        ...
                        <ol
                            ...
                            style="grid-template-rows: 1.75rem repeat(192,minmax(0, 1fr)) auto;"
                        >
                            <li
                                v-for="booking in bookings"
                                class="relative mt-px flex"
                                :style="`grid-row: ${(((booking.start.split(':')[0] - 8) * 60) + (booking.start.split(':')[1] *
                                        1)) / 5 + 2} / span ${booking.duration / 5}`"
                            >
                                <a
                                    href="#"
                                ...
                                >
                                    <p class="order-1 font-semibold text-blue-700">
                                        {{ booking.description }}
                                    </p><p class="order-1 text-blue-700">
                                        {{ booking.user.name }}
                                    </p>

                                    <p class="text-blue-500 group-hover:text-blue-700">
                                        <time datetime="2022-01-22T06:00">{{ booking.start }}</time>
                                    </p>
                                </a>
                            </li>
