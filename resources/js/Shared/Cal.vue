...
let selectedDate = ref(props.selectDate ? new Date(props.selectDate) : new Date());
let displayDate = ref(format(selectedDate.value, 'do MMM yyyy'));
let displayDay = ref(format(selectedDate.value, 'cccc'));
let displayMonth = ref(format(selectedDate.value,'MMMM'));
let displayYear = ref(format(selectedDate.value,'yyyy'));

let days = [];
let today = props.selectDate ? new Date(props.selectDate) : new Date();
let firstMonday =
...
dateRange.forEach( (date) => {
    days.push({
        ...
        'isSelected': isSameDay(today, date),
    });
});
...
onMounted(() => {
    ...
    container.value.scrollTop =
        ((container.value.scrollHeight - containerNav.value.offsetHeight - containerOffset.value.offsetHeight) *
            currentMinute) /
        1440;
    console.log(currentMinute, container)
})
...
function previousDay() {
    ...
    let stringDate = selectedDate.value.toISOString().split("T")[0];
...
<template>
    <div class="border rounded-md basis-9/12 flex max-h-[32rem] flex-col">
        <header class="flex flex-none items-center justify-between border-b border-gray-200 py-4 px-6">
            <div>
                <h1 class="text-lg font-semibold leading-6 text-gray-900">
                    <time @v-model="displayDate" :datetime="displayDate" class="sm:hidden">{{ displayDate }}</time>
                    <time :datetime="displayDate" class="hidden sm:inline">{{ displayDate }}</time>
                </h1>
                <p class="mt-1 text-sm text-gray-500">{{ displayDay }}</p>
            </div>
            <div class="flex items-center">
                <div class="flex items-center rounded-md shadow-sm md:items-stretch">
                    ...
                    <button type="button"
                            class="hidden border-t border-b border-gray-300 bg-white px-3.5 text-sm font-medium text-gray-700 hover:bg-gray-50 hover:text-gray-900 focus:relative md:block" @click="thisDay">
                        Today
                    </button>
                    <span class="relative -mx-px h-5 w-px bg-gray-300 md:hidden" />
                    <button type="button"
                            class="flex items-center justify-center rounded-r-md border border-l-0 border-gray-300 bg-white py-2 pl-4 pr-3 text-gray-400 hover:text-gray-500 focus:relative md:w-9 md:px-2 md:hover:bg-gray-50" @click="nextDay">
        ...
        <div class="isolate flex flex-auto overflow-hidden bg-white">
            <div ref="container" class="flex flex-auto flex-col max-h-full overflow-auto">
            ...
            <div class="hidden w-1/2 max-w-md flex-none border-l border-gray-100 py-10 px-8 md:block">
                <div class="flex items-center text-center text-gray-900">
                    ...
                    <div class="flex-auto font-semibold">{{ displayMonth }} {{ displayYear }}</div>
                    <button type="button" class="-m-1.5 flex flex-none items-center justify-center p-1.5 text-gray-400 hover:text-gray-500">
